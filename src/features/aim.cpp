...
static vec3_t get_closest_delta(vec3_t viewangles) {
    ...
    float best_fov    = cv_aimbot_fov->value;
...
void aimbot(usercmd_t* cmd) {
    /* Engine overwrites cmd->buttons after CL_CreateMove, so check
     * physical mouse state instead */
    bool attacking = (GetAsyncKeyState(VK_LBUTTON) & 0x8000) != 0;

    static int aim_log = 0;
    if (CVAR_ON(aimbot) && attacking && aim_log < 10) {
        aim_log++;
        printf("  aim#%d: atk=%d can_shoot=%d clip=%d nextAtk=%.3f nextPri=%.3f\n",
               aim_log, attacking, can_shoot(),
               g_iClip, g_flNextAttack, g_flNextPrimaryAttack);
    }

    if (!CVAR_ON(aimbot) || !attacking || !can_shoot())
        return;
    ...
    if (!vec_is_zero(best_delta)) {
        ...
        /* Use SetViewAngles so the engine actually applies our aim */
