...
static void autostrafe_rage(usercmd_t* cmd) {
    if (!i_pmove || i_pmove->movetype != MOVETYPE_WALK)
        return;
...
void bhop(usercmd_t* cmd) {
    static int bhop_entry = 0;

    if (!CVAR_ON(bhop) || !i_pmove || i_pmove->movetype != MOVETYPE_WALK)
        return;

    static int bhop_dbg = 0;
    static bool was_on_ground = true;

    bool on_ground = (i_pmove->flags & FL_ONGROUND) != 0;
    bool holding_jump = (cmd->buttons & IN_JUMP) != 0;

    bhop_dbg++;
    if (bhop_dbg <= 20) {
        printf("  bhop #%d: hold=%d onground=%d was_onground=%d\n",
               bhop_dbg, holding_jump, on_ground, was_on_ground);
    }

    if (holding_jump && !on_ground) {
        /* In air — release jump so engine registers a new press on landing */
        cmd->buttons &= ~IN_JUMP;
    }

    was_on_ground = on_ground;

    /* Autostrafe if enabled and in the air */
    if (holding_jump && !on_ground) {
